      ...
      <div v-for="(items, type) in grouped" :key="type" class="card p-5">
        <div class="flex items-center gap-2 mb-3">
          <span
            ...
            :class="typeColors[type] || typeColors['其他']"
            >{{ type }}</span
          >
        ...
        <div class="space-y-2">
          <div
            v-for="r in items"
            :key="r.id"
            class="flex items-center justify-between p-3 rounded-lg bg-slate-50/60 hover:bg-slate-50"
          >
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                <span class="text-sm font-medium text-slate-700">{{ r.activity_name }}</span>
                <span class="text-[10px] px-1.5 py-0.5 rounded bg-brand-50 text-brand-600">{{
                  skillLevelMap[r.skill_level] || r.skill_level
                }}</span>
              </div>
