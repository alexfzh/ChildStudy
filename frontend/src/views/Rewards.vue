...
const redeem = async (item) => {
  try {
    await ElMessageBox.confirm(
      `兑换「${item.reward.name}」需要 ${item.reward.cost_points} 积分，确认吗？`,
      "兑换奖励",
      { type: "warning" },
    )
...
const displayedLogs = computed(() => (showAllLogs.value ? filteredLogs.value : filteredLogs.value.slice(0, 5)))
...
<template>
  <div>
    ...
    <div v-if="ranks.length" class="card p-5 mb-5">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        <div
          ...
          :style="{ borderColor: tierColor(r.tier), backgroundColor: tierColor(r.tier) + '10' }"
        >
          <div class="text-2xl mb-1">{{ r.tier }}</div>
          <div class="text-xs text-slate-500 mb-1">{{ r.subject }}</div>
          <div class="text-xs" :style="{ color: tierColor(r.tier) }">
            {{ r.avg_score }}分 · ⭐{{ r.stars }} · {{ r.exam_count }}次
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              ...
              <div class="text-xs text-slate-400 mt-0.5">
                <span
                  class="inline-block px-1.5 py-0.5 rounded mr-1.5"
                  :class="{
    ...
    <div v-if="history.length" class="card p-5 mb-5">
          ...
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              <div class="text-sm font-medium text-slate-700 flex items-center gap-2">
                ...
                <span
                  class="shrink-0 inline-block px-1.5 py-0.5 rounded-full text-[10px] font-medium"
              ...
              <div class="text-xs text-slate-400 mt-0.5">
                兑换于 {{ h.earned_date
                }}<template v-if="(h.status || 'pending') === 'used' && h.used_at">
                  · 核销于 {{ fmtDateTime(h.used_at) }}</template
                >
              </div>
            </div>
          </div>
      ...
      <div
        v-if="historyFilter !== 'all' && filteredHistory.length === 0"
