<template>
  <div class="max-w-3xl mx-auto">
    ...
    <div v-else-if="phase === 'taking'" class="space-y-4">
      ...
      <!-- 题目卡片 -->
      <el-card v-if="currentQuestion" shadow="never" class="!border-slate-200">
        <div class="space-y-5">
          ...
          <div class="flex items-center justify-between pt-3 border-t border-slate-100">
            ...
            <div class="flex gap-2">
              <el-button :disabled="currentIndex === 0" size="default" class="!min-h-[44px]" @click="currentIndex--">
                ⬅ 上一题
              </el-button>
              <el-button
    ...
    <div v-else-if="phase === 'result'" class="space-y-6">
      <!-- 🎉 庆祝卡片：得分 + 用时 + 获得积分 + 新成就 -->
      <div
        class="rounded-2xl p-6 bg-gradient-to-br from-amber-50 via-white to-emerald-50 border-2 border-amber-200 shadow-soft"
      >
        <div class="text-center">
          ...
          <p class="text-sm text-slate-500 mt-1">
            ...
            <span v-if="result.time_spent" class="text-slate-400 ml-2"
              >· 用时 {{ formatSeconds(result.time_spent) }}</span
            >
          </p>
        </div>
        ...
        <!-- 积分 + 成就行 -->
        <div
          v-if="result.points_earned > 0 || (result.new_achievements && result.new_achievements.length > 0)"
          class="mt-5 flex flex-wrap items-center justify-center gap-2"
        >
          <span
            v-if="result.points_earned > 0"
          ...
          >
            ...
            <span class="text-xs text-amber-600 ml-1"
              >（今日 {{ result.daily_points_total }}/{{ result.daily_points_cap }}）</span
            >
          </span>
          <span
            v-for="ach in result.new_achievements || []"
            :key="ach.id"
      ...
      <div class="space-y-3">
        ...
        <div v-for="q in resultQuestions" :key="q.id" class="card p-4">
          <div class="flex items-start gap-3">
            <span
                  ...
                  <span
                    ...
                    :class="{
                      ...
                      'text-red-500 font-bold':
                        String.fromCharCode(65 + idx) === q.selected &&
      ...
      <div class="flex justify-center gap-3 pt-4">
        ...
        <el-button type="primary" size="large" class="!text-base !font-semibold !px-6" @click="restart"
          >再来一组</el-button
        >
      </div>
    </div>
