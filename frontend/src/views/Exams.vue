...
const scoreColor = (p) => p >= 85 ? "text-emerald-600" : p >= 70 ? "text-brand-600" : p >= 60 ? "text-amber-600" : "text-rose-600";
...
<template>
  ...
  <div v-else>
    ...
    <div class="card p-3 mb-4 flex gap-2 flex-wrap items-center">
      ...
      <el-select v-model="filterKnowledgePoint" placeholder="全部知识点" clearable size="default" class="!w-44" v-if="allKnowledgePoints.length">
    ...
    <div v-else class="space-y-2">
      <div
        v-for="e in exams"
        :key="e.id"
        class="card p-4 hover:shadow-soft transition-shadow"
      >
        <div class="flex items-start justify-between flex-wrap gap-2">
          <div class="flex items-start gap-3 flex-1 min-w-0">
            <div :class="['w-14 h-14 rounded-xl flex flex-col items-center justify-center text-white font-semibold', scoreBg(pct(e.score, e.full_score))]">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span class="badge bg-brand-50 text-brand-700">{{ {exam:'期中/期末',quiz:'单元测验',homework:'随堂测'}[e.exam_type] }}</span>
                <span v-if="e.target_score != null" class="badge" :class="e.score >= e.target_score ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'">
              ...
              <div class="text-xs text-slate-400 mt-1.5 flex flex-wrap gap-x-3 items-center">
                ...
                <span v-if="e.class_average != null" :class="['inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium', e.score >= e.class_average ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700']">
                  班均 {{ e.class_average }} · {{ e.score >= e.class_average ? '高于' : '低于' }}{{ Math.abs(e.score - e.class_average).toFixed(1) }}
                </span>
              </div>
              </div>
              <div v-if="e.target_score != null" class="mt-2">
                ...
                <div class="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                  <div class="h-full rounded-full transition-all" :class="e.score >= e.target_score ? 'bg-emerald-500' : 'bg-amber-500'" :style="`width:${Math.min((e.score / e.target_score) * 100, 100)}%`"></div>
                </div>
              </div>
          ...
          <div class="flex gap-1">
            <button class="btn-ghost text-xs text-brand-600" @click="router.push({ name: 'exam-analysis', query: { exam_id: e.id } })">分析</button>
      ...
      <el-form label-position="top">
        <div class="grid grid-cols-2 gap-3">
          <el-form-item label="科目" required>
            <el-select v-model="form.subject" filterable placeholder="从下拉选，不能新建" class="w-full" @change="fetchKnowledgePoints">
              <el-option v-for="s in subjectOptions" :key="s" :label="s" :value="s" />
            </el-select>
          ...
          <el-form-item label="目标分数">
            <el-input-number v-model="form.target_score" :precision="1" :step="1" :min="0" class="!w-full" placeholder="不设则留空" />
          </el-form-item>
          <el-form-item label="班级平均分">
            <el-input-number v-model="form.class_average" :precision="1" :step="1" :min="0" class="!w-full" placeholder="不设则留空" />
          </el-form-item>
        ...
        <el-form-item label="知识点标签">
          ...
          <div class="flex flex-wrap gap-1.5 mt-2">
            <span v-for="(k, i) in form.knowledge_points" :key="i" class="badge bg-amber-50 text-amber-700 cursor-pointer" @click="removeKp(i)">
              {{ k }} ×
            </span>
          </div>
          <div class="text-xs text-slate-400 mt-1">
            💡 从已有标签库选取，或直接输入新知识点后回车
          </div>
        </el-form-item>

        <el-form-item label="错题简述（便于后续分析）">
          <el-input v-model="form.wrong_questions" type="textarea" :rows="2" placeholder="如：应用题单位换算混淆、解方程移项忘记变号" />
        </el-form-item>
