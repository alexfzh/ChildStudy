...
const renderMarkdown = (md) => {
  ...
  // 表格 (简化：只识别 |---|)
  html = html.replace(/((?:\|[^\n]+\|\n)+)((?:\|[-:\s|]+\|\n))((?:\|[^\n]+\|\n?)+)/g, (m, header, sep, body) => {
    const ths = header.trim().split("|").slice(1, -1).map((s) => `<th>${s.trim()}</th>`).join("");
    const rows = body.trim().split("\n").map((line) => {
      const tds = line.split("|").slice(1, -1).map((s) => `<td>${s.trim()}</td>`).join("");
      return `<tr>${tds}</tr>`;
    }).join("");
  ...
  <div v-else class="space-y-5">
    ...
    <div class="card p-5 bg-slate-50 border-slate-200">
      ...
      <ol class="text-sm text-slate-600 space-y-1.5 list-decimal list-inside leading-relaxed">
        ...
        <li><strong>外部跑 AI</strong>：打开 <a class="text-brand-600 hover:underline" href="https://chat.deepseek.com" target="_blank">DeepSeek</a> / <a class="text-brand-600 hover:underline" href="https://kimi.moonshot.cn" target="_blank">Kimi</a> / <a class="text-brand-600 hover:underline" href="https://chatgpt.com" target="_blank">ChatGPT</a> / 其他 AI → 粘贴 → 等它生成报告 → 复制 AI 输出</li>
    ...
    <div class="card p-5">
      <div class="flex items-center justify-between mb-4">
        ...
        <button class="btn-ghost text-xs" :disabled="loading" @click="loadReports">{{ loading ? "加载中…" : "刷新" }}</button>
      </div>
        ...
        >
          <div class="flex-1 min-w-0">
            ...
            <div v-if="editingId === r.id" class="space-y-2" @click.stop>
              <el-input v-model="editTitle" placeholder="标题" @keyup.enter="saveEdit(r.id)" @keyup.esc="cancelEdit" @blur="saveEdit(r.id)" />
              <el-input v-model="editSummary" type="textarea" :rows="2" placeholder="摘要（可选）" @keyup.esc="cancelEdit" @blur="saveEdit(r.id)" />
              <div class="text-[11px] text-slate-400">按 Enter / 失焦保存，Esc 取消{{ saving ? ' · 保存中…' : '' }}</div>
            </div>
            <!-- view mode -->
          ...
          <div v-if="editingId !== r.id" class="flex items-center gap-1 flex-shrink-0">
            <button
              class="text-slate-400 hover:text-brand-600 text-xs"
              title="编辑"
    ...
    <el-dialog v-model="exportDialogVisible" title="导出当前数据为上下文" width="720px" top="5vh">
      <div class="space-y-3">
        ...
        <div class="text-xs text-slate-500">
          💡 复制后到外部 AI（DeepSeek / Kimi / ChatGPT 等）粘贴即可获得完整学情上下文。报告生成后，点下方"导入新报告"保存。
        </div>
      </div>
    </el-dialog>
    ...
    <el-drawer v-model="activeReport" :title="activeReport?.title || '报告详情'" size="640px" direction="rtl">
      <div v-if="activeReport" class="space-y-4">
        ...
        <div v-if="activeReport.summary" class="p-3 bg-brand-50 border border-brand-100 rounded-lg text-sm text-slate-700">
        ...
        <div class="border-t border-slate-100 pt-3 flex justify-end gap-2">
          <button class="btn-ghost text-sm" @click="navigator.clipboard.writeText(activeReport.raw_markdown); ElMessage.success('已复制原文')">📋 复制原文</button>
          <button class="btn-secondary text-sm text-rose-600" @click="deleteReport(activeReport.id, activeReport.title)">🗑 删除</button>
...
<style scoped>
.markdown-body :deep(h1) { font-size: 1.5rem; font-weight: 700; margin: 0.8em 0 0.5em; color: #1e293b; }
.markdown-body :deep(h2) { font-size: 1.25rem; font-weight: 700; margin: 0.8em 0 0.4em; color: #1e293b; border-bottom: 1px solid #e2e8f0; padding-bottom: 0.3em; }
.markdown-body :deep(h3) { font-size: 1.1rem; font-weight: 600; margin: 0.7em 0 0.3em; color: #334155; }
.markdown-body :deep(h4) { font-size: 1rem; font-weight: 600; margin: 0.6em 0 0.3em; color: #475569; }
.markdown-body :deep(p) { margin: 0.5em 0; line-height: 1.7; color: #334155; }
.markdown-body :deep(ul), .markdown-body :deep(ol) { padding-left: 1.5em; }
.markdown-body :deep(li) { margin: 0.25em 0; line-height: 1.6; color: #334155; }
.markdown-body :deep(code) { background: #f1f5f9; padding: 0.1em 0.3em; border-radius: 0.25em; font-size: 0.9em; color: #be185d; }
.markdown-body :deep(table) { border: 1px solid #e2e8f0; }
.markdown-body :deep(th), .markdown-body :deep(td) { border: 1px solid #e2e8f0; padding: 0.4em 0.8em; }
