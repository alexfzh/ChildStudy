...
export const reportsAPI = {
  ...
  listPeriod: (childId) => api.get("/reports/period/list", { params: { child_id: childId } }).then((r) => r.data),
...
export const quoteAPI = {
  random: (category) => api.get("/quotes/random", { params: category ? { category } : {} }).then((r) => r.data),
}
...
export const importExportAPI = {
  ...
  exportHomeworks: (params) =>
    api.get("/import-export/homeworks", { params, responseType: "blob" }).then((r) => r.data),
  importExams: (file, childId) => {
    const form = new FormData()
    form.append("file", file)
    if (childId) form.append("child_id", String(childId))
    return api
      .post("/import-export/exams", form, { headers: { "Content-Type": "multipart/form-data" } })
      .then((r) => r.data)
  },
  importHomeworks: (file, childId) => {
    ...
    if (childId) form.append("child_id", String(childId))
    return api
      .post("/import-export/homeworks", form, { headers: { "Content-Type": "multipart/form-data" } })
      .then((r) => r.data)
  },
}
...
export const questionBanksAPI = {
  ...
  updateQuestion: (bankId, qid, data) =>
    api.put(`/question-banks/${bankId}/questions/${qid}`, data).then((r) => r.data),
  ...
  submitExercise: (exerciseId, answers, timeSpent) =>
    api.post(`/question-banks/exercises/${exerciseId}/submit`, { answers, time_spent: timeSpent }).then((r) => r.data),
  getExercise: (exerciseId) => api.get(`/question-banks/exercises/${exerciseId}`).then((r) => r.data),
  listExercises: (childId) =>
    api.get("/question-banks/exercises", { params: { child_id: childId } }).then((r) => r.data),
...
export const studyProgressAPI = {
  getSummary: (childId, versionId) =>
    api.get(`/study-progress/child/${childId}/version/${versionId}`).then((r) => r.data),
...
export const projectWorksAPI = {
  ...
  uploadImage: (id, file) => {
    ...
    form.append("file", file)
    return api
      .post(`/project-works/${id}/upload`, form, { headers: { "Content-Type": "multipart/form-data" } })
      .then((r) => r.data)
  },
...
export const kpProgressAPI = {
  ...
  getForVersion: (childId, versionId) =>
    api.get(`/kp-progress/child/${childId}/version/${versionId}`).then((r) => r.data),
